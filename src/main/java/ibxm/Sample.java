...
public class Sample {
    ...
    public void resample_nearest(int sample_idx, int sample_frac, int step, int left_gain, int right_gain, int[] mix_buffer, int frame_offset, int frames) {
        ...
        while (offset <= end) {
            ...
            int n2 = offset;
            mix_buffer[n2] = mix_buffer[n2] + (this.sample_data[sample_idx] * left_gain >> 15);
            int n3 = offset + 1;
            mix_buffer[n3] = mix_buffer[n3] + (this.sample_data[sample_idx] * right_gain >> 15);
            offset += 2;
    ...
    public void resample_linear(int sample_idx, int sample_frac, int step, int left_gain, int right_gain, int[] mix_buffer, int frame_offset, int frames) {
        ...
        while (offset <= end) {
            ...
            int n2 = offset;
            mix_buffer[n2] = mix_buffer[n2] + (amplitude * left_gain >> 15);
            int n3 = offset + 1;
            mix_buffer[n3] = mix_buffer[n3] + (amplitude * right_gain >> 15);
            offset += 2;
            sample_idx += (sample_frac += step) >> 15;
            sample_frac &= 0x7FFF;
        }
